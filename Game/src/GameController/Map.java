			...
			for (int j = 1; j < tilesT - 1; j++) {
				...
				while (true) {
					...
					if (!edges.isEmpty()) {
						...
						Vector2f baseEdge = new Vector2f(currEdge.v2.x - currEdge.v1.x, currEdge.v2.y - currEdge.v1.y); // Points
																														// into
																														// tip
																														// of
																														// exploration
						for (GridAlignedEdge e : edges) {
							...
							if (bestChoice == null)
								doReplace = true;
							else if (Arithmetic.sign(cross) > Arithmetic.sign(bestCross))
								doReplace = true;
							else if (cross > 0 && dot < bestDot)
								doReplace = true;
							else if (bestCross < 0 && cross < 0 && dot > bestDot)
								doReplace = true;
...
//		System.out.println(edgeArr.size());
//
...
//		}
		...
		for (int i = 0; i < compEdges.length; i++) {
			...
			for (int j = 0; j < edgeSegs.length; j++) {
				...
				if (j + 1 < edgeSegs.length)
					nextEdge = edgeSegs[j + 1];
				else if (isLoop.get(i))
					nextEdge = edgeSegs[0];
	...
	private void logEdgeDuplicate(GridAlignedEdge e, ArrayList<GridAlignedEdge> connections,
			ArrayList<GridAlignedEdge> duplicates) {
